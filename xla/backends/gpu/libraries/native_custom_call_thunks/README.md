...
An HLO instruction and a kernel describe their buffers in very different terms,
and getting from one to the other goes through three index spaces that are
easily confused:

1.  **Operands and the result of the HLO instruction.** What the frontend
    wrote. An instruction takes any number of operands but produces exactly one
    result. That result may be a tuple, in which case it has to be decomposed
    into its array leaves - one buffer per leaf - before a kernel can be handed
    anything.
2.  **Positions in `emitters::KernelArguments`.** A kernel draws no distinction
    between an input and an output: it takes one flat, ordered list of buffers
    and writes to whichever of them it was told to write to.
    `emitters::KernelArguments` is that list - operands in operand order, then
    the array leaves of the result shape in shape-index order. Which of them
    are written is tracked separately, by the `written` flag on each argument.
    `CustomKernelThunk` reports its buffer uses in this order, and
    `zeroed_output_buffer_indices` is expressed in it.
3.  **The kernel's parameter list.** The kernel's actual signature, which need
    not be the list from (2): it may reorder the buffers, mention only some of
    them, and interleave by-value arguments between them. It is defined by the
    `stream_executor::KernelArgsPackingSpec` that the kernel spec carries, whose
    relocations refer to positions in (2). It has to be a packing spec rather
    than a packing function, because only the former can be serialized for AOT
    compilation.

Get from (1) to (2) with `KernelArguments::PositionOfOperand` and
`PositionOfResult` rather than by counting arguments yourself; that stays
correct when an operand is added or when the result shape gains a leaf. For the
common single-result custom call, `SingleResultArgumentPosition(instr,
kernel_args)` does both steps at once.
...
A handler runs at compile time, when the target device is not available: there
is no device and no stream to query. An FFI handler's `Instantiate` stage runs
under the same restriction; what a native handler has no counterpart for is
`Initialize` and `Execute`, which do get a stream and with it the device.

Every decision that depends on the target device therefore has to be made
without it. Kernel selection is the usual case: code that would normally
inspect the current device has to be refactored to take the device's properties
as a parameter.

`ctx.GetDeviceDescription()` provides those properties - a complete description
of the device the module is being compiled for, and the authoritative source
for such decisions.

For a CUDA-only handler, `GetCudaComputeCapability(ctx)` is a convenience
wrapper that pulls just the compute capability out of that description.
...
-   **A handler cannot decline.** Returning an error fails the compilation;
    ...
    A handler must therefore be able to lower every instance of its target. We
    may add a way to decline an instruction and fall back to the FFI path in
    the future.
